 ...
 * */
void init(HuffmanTree* huffmanTree, uint64 *s, size_t length)
{
	//FuncName;
        //printf("init\n");
	size_t i, index;
	...
	//uint64 *freq = (uint64 *)malloc(64*sizeof(uint64)); // maximum 64 bit planes to encode.
	//memset(freq, 0, 64*sizeof(uint64));
        
	// find out unique set of s
	//printf("length=%lu\n", length);
	uint64* uarray = malloc(length * sizeof(uint64));
	int k = 0;
	int j;
	for (i=0; i<length; i++)
	{
	...
	// calculate frequence
        int *freq = (int*)malloc(64*sizeof(int));
        memset(freq, 0, 64*sizeof(int));
